...
class MySearchView(SearchView):
    ...
    def get_queryset(self):
        ...
        if 'group' in self.request.GET:
            queryset = SearchQuerySet().\
                filter(content=AutoQuery(self.request.GET['q']), main_group_slug=self.request.GET.get('group'))
        return queryset
    ...
    def get_context_data(self, **kwargs):
        ...
        queryset = SearchQuerySet().filter(content=AutoQuery(self.request.GET['q']))
...
def autocomplete(request):
    ...
    suggestions = [{'title': obj.title,
                    'main_image': obj.main_image,
                    'main_group_title': obj.main_group_title,
                    'year_release': defaultfilters.date(obj.year_release, 'Y'),
                    'countries': defaultfilters.truncatechars(', '.join(obj.countries), 30),
                    'genres': ', '.join(obj.genres)} for obj in sqs]
    the_data = json.dumps({
        'results': suggestions
    })
